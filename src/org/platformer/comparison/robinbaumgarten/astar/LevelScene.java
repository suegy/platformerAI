package org.platformer.comparison.robinbaumgarten.astar;
...
import org.platformer.comparison.robinbaumgarten.astar.level.Level;
import org.platformer.comparison.robinbaumgarten.astar.level.SpriteTemplate;
import org.platformer.comparison.robinbaumgarten.astar.sprites.*;
    ...
    public boolean setLevelScene(byte[][] data)
    {
        ...
        for (int y = MarioYInMap - HalfObsHeight, obsX = 0; y < MarioYInMap + HalfObsHeight; y++, obsX++)
        {
            for (int x = MarioXInMap - HalfObsWidth, obsY = 0; x < MarioXInMap + HalfObsWidth; x++, obsY++)
            {
                ...
                {
                    byte datum = 0;
                    if (obsX > 0 && obsY > 0 && data.length > obsX && data[obsX].length > obsY)
