...
public class AStarSimulator 
{
    ...
    private boolean[] createAction(boolean left, boolean right, boolean down, boolean jump, boolean speed)
    {
    	...
    	action[Plumber.KEY_DOWN] = down;
    	action[Plumber.KEY_JUMP] = jump;
    	action[Plumber.KEY_LEFT] = left;
    	action[Plumber.KEY_RIGHT] = right;
    	action[Plumber.KEY_SPEED] = speed;
		action[Plumber.KEY_UP] = false;
    	return action;
    }
    ...
    {
    	float dist = 0;
    	float runningSpeed =  action[Plumber.KEY_SPEED] ? 1.2f : 0.6f;
    	int dir = 0;
    	if (action[Plumber.KEY_LEFT]) dir = -1;
    	if (action[Plumber.KEY_RIGHT]) dir = 1;
    ...
    private ArrayList<boolean[]> extractPlan()
    {
    	...
    	while (current.parentPos != null)
    	{
    		...
    		if (levelScene.verbose > 2) 
    			System.out.print("[" 
    				+ (current.action[Plumber.KEY_DOWN] ? "d" : "")
    				+ (current.action[Plumber.KEY_RIGHT] ? "r" : "")
    				+ (current.action[Plumber.KEY_LEFT] ? "l" : "")
    				+ (current.action[Plumber.KEY_JUMP] ? "j" : "")
    				+ (current.action[Plumber.KEY_SPEED] ? "s" : "")
    				+ (current.hasBeenHurt ? "-" : "") + "]");
    ...
    {
    	String s = "";
    	if (action[Plumber.KEY_RIGHT]) s+= "Forward ";
    	if (action[Plumber.KEY_LEFT]) s+= "Backward ";
    	if (action[Plumber.KEY_SPEED]) s+= "Speed ";
    	if (action[Plumber.KEY_JUMP]) s+= "Jump ";
    	if (action[Plumber.KEY_DOWN]) s+= "Duck";
		return s;
    }
	...
	public void advanceStep(boolean[] action)
	{
		...
		if (levelScene.verbose > 8) System.out.print("[" 
				+ (action[Plumber.KEY_DOWN] ? "d" : "")
				+ (action[Plumber.KEY_RIGHT] ? "r" : "")
				+ (action[Plumber.KEY_LEFT] ? "l" : "")
				+ (action[Plumber.KEY_JUMP] ? "j" : "")
				+ (action[Plumber.KEY_SPEED] ? "s" : "") + "]");
		levelScene.tick();
	}
